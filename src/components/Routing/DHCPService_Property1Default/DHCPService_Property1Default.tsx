...
export const DHCPService_Property1Default: FC<Props> = memo(function DHCPService_Property1Default(props = {}) {
  ...
  return (
    <>
    <button id = "DHCPService" className={classes.sidebar_button} onClick={() => setIsOpen((prev)=>!prev)} style={!isOpen ? {} : {background: '#0D597F', color: 'white', fontWeight: '700'} } >DHCP Service
        {!isOpen ? (
        ...
        <div className={classes.div_bar}>
          <button id='DHCPService-1' onClick={() => click1("DHCPService-1", ComponentOne)} className={classes.inner_sidebar_button}>•  DHCP Service</button>
